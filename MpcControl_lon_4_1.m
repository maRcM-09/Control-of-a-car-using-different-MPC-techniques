classdef MpcControl_lon < MpcControlBase
    ...
    methods
        ...
        function ctrl_opti = setup_controller(mpc)
            ...
            % first we get our target reference 
            P1 = [F; M*K];
            p2 = [f;m+M*(K*xs-us)];
            ...
            % Disturbance factor
            B_dist = 1;
            S_u = 5; % smoothness term for the input to avoid sudden changes 
            for i = 1:N-1
                % system dynamics
                con = con + (X(:,i+1) == A*(X(:,i)-xs) + B*(U(:,i)-us) + B_dist*d_est + f_xs_us);
                % input constraints
                con = con + (M*U(:,i) <= m); 
                % state constraints
                con = con + (F*X(:,i) <= f);
        ...
        function [Vs_ref, us_ref] = compute_steady_state_target(mpc, ref, d_est)
            ...
            % YOUR CODE HERE YOUR CODE HERE YOUR CODE HERE YOUR CODE HERE
            Vs_ref = ref;
            us_ref = us + ((1-A)*(Vs_ref-xs)-B_dist*d_est)/B;
            % YOUR CODE HERE YOUR CODE HERE YOUR CODE HERE YOUR CODE HERE
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        end
    end
