classdef NmpcControlOvertake < handle
    ...
    methods
        function obj = NmpcControlOvertake(car, H)
            ...
            for k = 1:N_segs
                ...
                if k < N_segs
                    opti.subject_to(-0.1 <= obj.U(1, k+1) - obj.U(1, k)<=0.1);
                    opti.subject_to(-0.5 <= obj.U(2, k+1) - obj.U(2, k)<=0.5);
                end
                ...
                %-------------------------------------------------------------------------------
                % Position of ego car
                p_ego = [obj.X(1, k); obj.X(2, k)]; % [x; y] position from the state vector

                % Position of lead car travelling at constant speed
                x_lead = obj.x0other(1) + k * Ts * obj.x0other(4); 
                y_lead = obj.x0other(2); 
                p_lead = [x_lead-2.2; y_lead];
                % Ellipse constraint
                opti.subject_to((p_ego - p_lead)' * H * (p_ego - p_lead) >= 1);
                %--------------------------------------------------------------------------------
